...
def get_media_dir_for_date(date: datetime) -> str:
    """Get the media directory for a specific date"""
    date_str = date.strftime("%Y-%m-%d")
    media_path = os.path.join(settings.MEDIA_DIR, date_str)
    os.makedirs(media_path, exist_ok=True)
    return media_path

async def save_photo_async(frame: np.ndarray, filename: str) -> str:
    """Save a photo frame to disk and create thumbnail asynchronously"""
    # Create directory for today
    date_dir = get_media_dir_for_date(datetime.now())
...
async def start_video_recording_async(camera, filename: str) -> str:
...
async def stop_video_recording_async(camera) -> str:
...
async def get_media_by_date_async(start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
    """Get media items grouped by date asynchronously"""
    result = []
    loop = asyncio.get_event_loop()
    
    # Function to process a single date directory
    async def process_date_dir(date_str):
        date_dir = os.path.join(settings.MEDIA_DIR, date_str)
        ...
        for meta_file in meta_files:
            ...
            try:
                ...
                item = {
                    ...
                    "type": metadata['type'],
                    "date": date_str,
                    "thumbnail": f"/media/{date_str}/{thumb_filename}",
                    "url": f"/media/{date_str}/{filename}",
                    "metadata": metadata
                }
                items.append(item)
    ...
    # Process all dates in the range concurrently
    tasks = []
    current_date = start_date
    while current_date <= end_date:
        date_str = current_date.strftime("%Y-%m-%d")
        tasks.append(process_date_dir(date_str))
    ...
    result = [r for r in date_results if r is not None]
    
    return result

async def get_media_info_async(media_id: str) -> Optional[Dict[str, Any]]:
    ...
    # Search for the media ID in all date directories
    for date_dir in date_dirs:
        date_path = os.path.join(settings.MEDIA_DIR, date_dir)
        ...
        if exists:
            try:
                ...
                return {
                    ...
                    "type": metadata['type'],
                    "date": date_dir,
                    "thumbnail": f"/media/{date_dir}/{thumb_filename}",
                    "url": f"/media/{date_dir}/{filename}",
                    "metadata": metadata
                }
            except Exception:
                pass
...
def save_photo(frame: np.ndarray, filename: str) -> str:
    """Synchronous version of save_photo_async for backwards compatibility"""
    loop = asyncio.new_event_loop()
    try:
        return loop.run_until_complete(save_photo_async(frame, filename))
    finally:
        loop.close()

def start_video_recording(camera, filename: str) -> str:
    ...
    camera.start_recording(os.path.join(get_media_dir_for_date(datetime.now()), filename))
    return os.path.join(get_media_dir_for_date(datetime.now()), filename)

def stop_video_recording(camera) -> str:
    """Synchronous version of stop_video_recording_async for backwards compatibility"""
    loop = asyncio.new_event_loop()
    try:
        return loop.run_until_complete(stop_video_recording_async(camera))
    finally:
        loop.close()

def get_media_by_date(start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
    """Synchronous version of get_media_by_date_async for backwards compatibility"""
    loop = asyncio.new_event_loop()
    try:
        return loop.run_until_complete(get_media_by_date_async(start_date, end_date))
    finally:
        loop.close()

def get_media_info(media_id: str) -> Optional[Dict[str, Any]]:
    """Synchronous version of get_media_info_async for backwards compatibility"""
    loop = asyncio.new_event_loop()
    try:
        return loop.run_until_complete(get_media_info_async(media_id))
    finally:
        loop.close()